...
def main() -> None:
    ...
    parser.add_argument("command", nargs="?", default="bot", choices=["bot", "report", "aviation", "nowcast", "sources", "result"])
    ...
    if args.command == "nowcast":
        print(asyncio.run(service.render_nowcasting(target_date=target)))
        return
