...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await _reply(
        ...
        "/today /aviation /ltac /now /metar /taf /models /nowcast /market /edge /backtest /sources /chart /result",
    )
...
async def nowcast(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    service = service_from_context(context)
    if not _is_allowed_chat(update, service):
        return
    target = _parse_date_arg(context.args) if context.args else None
    await _reply_long(update, await service.render_nowcasting(target_date=target))
