...
from src.forecast.ltac_microclimate import is_ltac_westerly_asphalt_wind
...
class TelegramReportRenderer:
    ...
    def _dynamic_lines(self, analysis: ForecastAnalysis) -> list[str]:
        ...
        if microclimate is not None and abs(microclimate.value_c) >= 0.05:
    ...
    def _rounding_alarm_lines(
        ...
        rounded_c = math.floor(market_boundary_c) + 1
        return [
            _bullet(
...
def _wind_thermal_effect(direction_deg: int | None) -> str:
    ...
    if is_ltac_westerly_asphalt_wind(direction_deg):
